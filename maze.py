...
class Maze:
    ...
    def _solve_r(self, i, j) -> bool:
        ...
        # determine which cell(s) to visit next
        # left
        if i > 0 and not self.__cells[i - 1][j].has_right_wall and not self.__cells[i - 1][j].visited:
        ...
        # right
        if i < self.__num_cols - 1 and not self.__cells[i + 1][j].has_left_wall  and not self.__cells[i + 1][j].visited:
            self.__cells[i][j].draw_move(self.__cells[i + 1][j])
            if self._solve_r(i + 1, j):
                return True
            self.__cells[i][j].draw_move(self.__cells[i + 1][j], True)
        ...
        # up
        if j > 0 and not self.__cells[i][j - 1].has_bottom_wall  and not self.__cells[i][j - 1].visited:
        ...
        # down
        if j < self.__num_rows - 1 and not self.__cells[i][j + 1].has_top_wall and not self.__cells[i][j + 1].visited:
            self.__cells[i][j].draw_move(self.__cells[i][j + 1])
            if self._solve_r(i, j + 1):
                return True
            self.__cells[i][j].draw_move(self.__cells[i][j + 1], True)
